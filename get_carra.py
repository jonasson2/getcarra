...
def read_grib(grib_file, variables: list[str], height_levels: list[int]):
...
for (date, loc) in date_location.items():
    grib_file = retrieve_specific_time(date, height_lev, var_list, prod_type)
    time.sleep(1)
    res, lat_grid, lon_grid = read_grib(grib_file, var_list, height_lev)
